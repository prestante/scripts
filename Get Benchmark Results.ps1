...
$result68 = Get-Content 'C:\PS\Report68.txt' | where {$_ -match 'Epyc 9124'} | % {$_ -match '\d+.{0,10}$' | Out-Null ; $Matches.Values -replace ' .*'}
...
"Test,WTL-HPY-345-6" | Out-File $csvfile -Append ascii
for ($i=0; $i-lt $list.Length; $i++) {
    "{0},{1}" -f $list[$i], $result68[$i] | Out-File $csvfile -Append ascii
}
