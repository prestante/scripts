            ...
            if ($KnownBodies -notcontains ([System.BitConverter]::ToString($md5.ComputeHash($utf8.GetBytes($body))))) {
                ...
                [regex]::Matches($body,$regexBody) | % {
                    ...
                    if ($type -eq 'Request') {
                        ...
                        if (($command -eq 'get all items') -or ($command -eq 'get loaded layouts') -or ($command -eq 'get system settings') -or ($command -eq 'get all salvos') -or ($command -eq 'get all layouts')) {$systemRequest=1; $f=9; $b=0} else {$systemRequest=0; $f=11; $b=9}
                    ...
                    if ($type -eq 'Response') {
                        ...
                        if (($command -eq 'get all items') -or ($command -eq 'get loaded layouts') -or ($command -eq 'get system settings') -or ($command -eq 'get all salvos') -or ($command -eq 'get all layouts')) {$systemResponse=1} else {$systemResponse=0}
