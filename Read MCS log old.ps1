...
do {
    if (!$pause) {
        ...
        $regex = '(?s)(?<body>(?<time>\d\d\/\d\d\/\d\d\d\d \d\d:\d\d:\d\d:\d\d)  (?<direction>[=<>]{3})(?! \nH).*?(?<mcsmessage><MCSMessage.*?<\/MCSMessage>))'
        [regex]::Matches($content,$regex) | % {
            #$kk
            $body = $_.groups['body'].value
            $time = $_.groups['time'].value
            $direction = $_.groups['direction'].value
            $mcsmessage = $_.groups['mcsmessage'].value
            
            #"body = `n$body`n"
            #"time = `n$time`n"
            #"direction = `n$direction`n"
            #"mcsmessage = `n$mcsmessage`n"
            ...
            if ($KnownBodies -notcontains ([System.BitConverter]::ToString($md5.ComputeHash($utf8.GetBytes($body))))) {
                ...
                if ($direction -eq '==>') {
                    ...
                    if (($showSystem) -or !($systemRequest)) {
                        if ($category -ne 'AudioMixer') {
                            Write-Text "$time ==> $command - $(if($target){" TargetState:$target"})$(if($instance){" InstanceID:$instance"})$(if($source){" SourceID:$source"})$(if($showSystem){" ($id)"})" -f $f -b $b
                            $waitID = $id
                        }
                    }
                }
                ...
                elseif ($direction -eq '<==') {
                    ...
                    [regex]::Matches($mcsmessage,$leftRegex) | % {
                        $id = $_.groups['id'].value
                        $command = $_.groups['command'].value
