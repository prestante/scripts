...
function Title {"--------`nPress <Space> or <Enter> to show/hide the graph.`nDon't panic. It takes some time for the graph to appear.`nThis powershell window shold be in focus for keybinds to work. So don't click graph window.`nPress <Esc> to exit.`n--------"}
...
#$timer.Enabled = $true
$Form.Text = "Press <Space> or <Enter> to close the graph or <Escape> to exit"
$Form.Width = $width
$Form.Height = $height
...
#$SaveButton.add_click({$Form.Close()})
...
do {
    ...
    if ($host.ui.RawUi.KeyAvailable) {
        ...
        switch ($key.VirtualKeyCode) {
            13 {if ($form.Visible) {$Form.Hide()} else {$Form.Show()}} #<Enter>
            82 {} #<R>
            27 {exit} #<Esc>
            112 {Title} #<F1>
            73 {}
            65 {} #<A>
            32 {if ($form.Visible) {$Form.Hide()} else {$Form.Show()}} #<Space>
        } #end switch
    } #end if
    #$Host.UI.RawUI.FlushInputBuffer()
