#restore the last build LISTCONF.INI from the 12.28.11 one
...
Invoke-Command -ComputerName $CTC -Credential $Creds {
    $buildFolder = Get-ChildItem ('\\localhost\server\') | where {$_.name -match '12.28.13'} | select -First 1 -ExpandProperty FullName
    $OldInstallPath = (gci 'C:\server\' | where {$_.Name -match '12.28.11'} | select -First 1).FullName
    Copy-Item ($OldInstallPath | gci | where {$_.name -cmatch 'INI'}).FullName -Destination $buildFolder -Force
