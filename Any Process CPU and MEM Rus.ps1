if ((Get-WinSystemLocale).DisplayName -notmatch 'русск') {Write-Host 'Системная локаль не русская. Попробуйте другой скрипт'; Sleep 3; return}Write-Host "Reading CPU properties..." -fo Yellow -ba Black
...
function updProcs {
    ...
    if (Compare-Object $Global:lastProcesses $Global:Processes) { #processes list has been changed
        ...
        Get-Counter -ListSet процесс | Out-Null #just to refresh system counters data
...
Function updCounters {
    ...
    foreach ($uniq in ($Global:Processes.Name | select -Unique)) {
        0..((Get-Process -Name $uniq).Count - 1) | %{
            $countersList.Add("\процесс($uniq#$_)\Идентификатор процесса"); 
            $countersList.Add("\процесс($uniq#$_)\Рабочий набор")
            $countersList.Add("\процесс($uniq#$_)\% загруженности процессора"); 
        }
    }
    $countersList.Add("\память\Доступно байт")
    $countersList.Add("\процесс(idle)\% загруженности процессора")
    ...
    $counterResults | ?{$_.path -match 'Идентификатор процесса'} | %{
