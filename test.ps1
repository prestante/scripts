...
while ( $true ) {  # Main cycle
    ...
    if ( $JobData.State -eq 'Completed' -or $JobData.State -eq 'Failed' -or -not $JobData ) {
        if ( $Table.Where({$null -ne $_.Ping}) ) { $JobData = Invoke-Command -ComputerName $Table.Where({$null -ne $_.Ping}).ServerName -ScriptBlock $JobDataScript -Credential $CredsDomain -AsJob }
    }
