...
Invoke-Command -ComputerName $CTC -ArgumentList $InstallAppVersion, $PrevAppVersion, $CredsDomain, $BuildsFolder -Credential $CredsDomain {
    ...
    $Parameters = "\s \server`"$DSname`""  # silent install

    $report += "`n`t File: $File"
    $report += "`n`t Parameters: $Parameters"

    if ( -not (Test-Path $File.FullName) ) { $report += "Installer not found: $($File.FullName)" ; return }
...
<#
    ...
    if (!(Get-ItemProperty HKLM:SOFTWARE\Wow6432Node\Microsoft\Windows\CurrentVersion\Uninstall\* | where {$_.displayname -match 'ADC.*Server' -and $_.displayversion -match $InstallAppVersion})) {
        Write-Host "$($HostName): Installing $($File.VersionInfo.ProductName)..."
        ...
        Write-Host "$($HostName) - DS $InstallAppVersion already exists." -fo Yellow -ba Black
    }
#>
