...
#main cycle to operate with every CTC PC one by one
Invoke-Command -ComputerName $CTC -Authentication Credssp -Credential $Creds -ScriptBlock {

    #just echoing current CTC PC name and IP
    $env:COMPUTERNAME
        
    New-SmbShare -Name "Imagine Communications" -Path "C:\Program Files (x86)\Imagine Communications" -FullAccess "Everyone"
}