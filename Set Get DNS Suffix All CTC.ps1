...
Invoke-Command $CTC -Credential $Creds {
    ...
    if (!(Get-DnsClientGlobalSetting).SuffixSearchList) { Set-DnsClientGlobalSetting -SuffixSearchList @("tecom.nnov.ru") }
    Write-Host "$($env:COMPUTERNAME) : $((Get-DnsClientGlobalSetting).SuffixSearchList)"
}
