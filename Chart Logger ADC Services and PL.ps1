$Processes = 
'Harris.Automation.ADC.Services.ADCDataServiceHost',
'Harris.Automation.ADC.Services.AggregationServiceHost',
'Harris.Automation.ADC.Services.AsRunServiceHost',
'Harris.Automation.ADC.Services.CommercialValidationServiceHost',
'Harris.Automation.ADC.Services.DeviceServiceHost',
'Harris.Automation.ADC.Services.ErrorReportingServiceHost',
'Harris.Automation.ADC.Services.IntegrationServiceHost',
'Harris.Automation.ADC.Services.ListServiceHost',
'Harris.Automation.ADC.Services.ManagerServiceHost',
'Harris.Automation.ADC.Services.MaterialServiceHost',
'Harris.Automation.ADC.Services.SecurityServiceHost',
'Harris.Automation.ADC.Services.SynchroServiceHost',
'Harris.Automation.ADC.Services.TimecodeServiceHost',
'Playlist';
$ProcNames = $Processes -replace 'Harris.Automation.ADC.Services.','' -replace 'Host','' -replace 'Service','' -replace 'Validation',''

$logfile="C:\PS\logs\ServicesPL $(Get-Date -Format 'yyyy-MM-dd HH-mm-ss').csv"
...
Function mem {
    foreach ($Process in $Processes) {
        try {
            ...
            else {[int][math]::Round(((Get-Process $Process -ea Stop).WorkingSet64) / 1MB)}
        }
        catch { 0 }
    }
}
...
do {
    ...
    if ($host.ui.RawUi.KeyAvailable) {
        ...
        switch ($key.VirtualKeyCode) {
            ...
            <#O#> 79 {
                $o = Get-FileName 'C:\PS\logs'
                if ($o) {$j = Start-job $function:NewGraph -ArgumentList ($o)}
            }
        } #end switch
    } #end if
    #$Host.UI.RawUI.FlushInputBuffer()
